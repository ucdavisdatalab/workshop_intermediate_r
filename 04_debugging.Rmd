...
### Debugging Case Study: Boottrapping Average Penguin Mass
...
Now imagine that you are trying to use a script that was written by a previous member of your lab. Let's copy it from its home on the web, [here](github.com/ucdavisdatalab/workshop_intermediate_r/) to a new blank script on our computer(s). The script provides a function that splits the penguins into groups according to some grouping factors like species and sex, then plots the uncertainty in the average penguin mass per group. The plot is show below for mass by species.
